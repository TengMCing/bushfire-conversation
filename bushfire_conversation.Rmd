...
From our model predictions, using satellite hotspots data, weather, distance to roads, camp sites, fire stations, and fuel load, suggests that 80% of the fires in Victoria during the 2019-2020 season can be attributed to lightning. Only 4% due to arson, 14% through accidents, and 1% from burning off. 
...
p3 <- training %>%
  ggplot() +
      ...
      geom_text(data=training_p, aes(x=lon, y=lat, label=freq), size=6) + 
...
p4 <- ggplot() +
  ...
  scale_colour_brewer(palette = "RdBu") +
  facet_wrap(~cause, ncol=4) +
  geom_text(data=pred_p, aes(x=lon, y=lat, label=freq), size=6) + 
...
p5 <- prediction %>%
  ...
  ggplot() +
  geom_sf(data = vic_map, fill = NA, col = "black") +
  geom_point(aes(x=lon, y=lat, colour = cause)) +
  scale_colour_brewer(palette = "RdBu") +
...
Close to Melbourne there was disturbing news of a bushfire in a high density koala habitat on French Island Jan 18. [Mornington Peninsula News](http://www.mpnews.com.au/2020/01/21/all-clear-after-island-fire-fright/) reports: 
...
Our model predicted this to be arson (but remember arson is difficult to distinguish from accident), so it disagrees. If we zoom into a map of the area to the first detected hotspot it can be seen to be right on a road, which makes it plausible for this to actually have been a human-caused fire.   
